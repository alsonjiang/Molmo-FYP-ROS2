...
class YoloHttpAdapter(Node):
    def __init__(self):
        ...
        # classes: integer array. [] = all, [0] = person only
        ...
        self.yolo_url: str = str(self.get_parameter("yolo_url").value)
        ...
        self.timeout_s: float = float(self.get_parameter("timeout_s").value)
        self.jpeg_quality: int = int(self.get_parameter("jpeg_quality").value)

        classes_val = self.get_parameter("classes").value
        # rclpy may return a tuple-like; force list[int]
        self.classes: List[int] = [int(x) for x in list(classes_val)]
        ...
        self.get_logger().info(f"rate_hz={self.rate_hz} conf={self.conf} iou={self.iou} timeout_s={self.timeout_s}")
        self.get_logger().info(f"classes={self.classes} (empty list means all)")
    ...
    def _infer(self, bgr: np.ndarray):
        try:
            ...
            if not ok:
                self.get_logger().warn_throttle(2.0, "Failed to JPEG-encode frame")
                return

            # YOLO service expects comma-separated string; empty means all classes
            classes_str = ",".join(str(x) for x in self.classes) if len(self.classes) > 0 else ""

            files = {"image": ("frame.jpg", jpg.tobytes(), "image/jpeg")}
            params = {"conf": self.conf, "iou": self.iou, "classes": classes_str}
